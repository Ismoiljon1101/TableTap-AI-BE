...
async function fix() {
    ...
    for (const user of users) {
        ...
        if (!rid || !Types.ObjectId.isValid(rid.toString())) {
            console.log(`  ❌ User ${user.get('email')} had invalid RID: ${rid}. Fixing to ${validRestaurant._id}`);
            await UserModel.updateOne({ _id: user._id }, { $set: { restaurantId: validRestaurant._id } });
        } else {
            // Check if it exists
            const exists = await RestaurantModel.findById(rid);
            if (!exists) {
                console.log(`  ❌ User ${user.get('email')} had non-existent RID: ${rid}. Linking to ${validRestaurant._id}`);
                await UserModel.updateOne({ _id: user._id }, { $set: { restaurantId: validRestaurant._id } });
            }
        }
    }
