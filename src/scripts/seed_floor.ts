...
const SectionSchema = new Schema(
  { name: String, restaurantId: Types.ObjectId, code: String },
  { strict: false },
);
...
async function seed() {
  ...
  const uniqueRestaurantIds = Array.from(
    new Set(users.map((u) => u.restaurantId?.toString()).filter((id) => !!id)),
  );

  console.log(
    `👤  Found ${users.length} users across ${uniqueRestaurantIds.length} unique restaurant IDs.\n`,
  );
  ...
  for (const ridStr of uniqueRestaurantIds) {
    ...
    for (const s of SECTIONS) {
      ...
      sectionIdMap.set(s.code, doc._id);
    }
