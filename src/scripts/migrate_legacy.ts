...
async function migrate() {
    ...
    for (const table of tables) {
        ...
        if (typeof currentSec === 'string') {
            ...
            if (!secId) {
                ...
                let sectionDoc = await SectionModel.findOne({ restaurantId: table.get('restaurantId'), name: secName });
                if (!sectionDoc) {
                ...
                secId = sectionDoc._id as Types.ObjectId;
    ...
    for (const item of items) {
        ...
        if (typeof currentCat === 'string') {
            ...
            if (!catId) {
                let catDoc = await CategoryModel.findOne({ restaurantId: item.get('restaurantId'), name: catName });
                if (!catDoc) {
                ...
                catId = catDoc._id as Types.ObjectId;
