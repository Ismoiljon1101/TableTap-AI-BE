...
import { CreateTableDto, UpdateTableDto, CreateTablesDto } from './dto/table.dto';
...
export class TablesService {
    ...
    private async calcDefaultPosition(restaurantId: string | Types.ObjectId): Promise<{ x: number; y: number }> {
        ...
        const COLS = 4;
        const STEP = 3;
        const count = await this.tableModel.countDocuments({ restaurantId: rid }).exec();
