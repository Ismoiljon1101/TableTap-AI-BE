...
export class AuthService {
    ...
    async register(registerDto: RegisterDto) {
        const { email, password, nickname, restaurantName, restaurantId, role } = registerDto;
        ...
        if (userRole === UserRole.OWNER || userRole === 'owner' as any) {
            if (!restaurantName) {
                throw new UnauthorizedException('Restaurant name is required for owners');
            }
        ...
        // Waiter joins existing restaurant
            ...
            if (!restaurantId) {
                throw new UnauthorizedException('Restaurant ID is required for waiters');
            }
    ...
    async googleAuth(googleAuthDto: GoogleAuthDto) {
        const { googleId, email, nickname, restaurantId, restaurantName } = googleAuthDto;
        ...
        if (!user) {
            ...
            if (restaurantId) {
                ...
                throw new UnauthorizedException('Restaurant information required for new users');
            }
